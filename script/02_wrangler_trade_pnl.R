...
## description ##
#################
# this script calculates the weekly trades and the unscaled PNLs starting 
# from the chain-description, chain-history, and option-history.
...
############################
df_underlying  <- tibble(underlying = c("GLD"))
...
df_chain <- 
    read_csv(
        "data_output/monthly/gld_monthly_2014_2018_chain_desc.csv"
        , col_types = cols()
    )

df_chain_hist <- 
    read_csv(
        "data_output/monthly/gld_monthly_2014_2018_chain_hist.csv"
        , col_types = cols()
    )

df_opt_hist <-
    read_csv(
        "data_output/monthly/gld_monthly_2014_2018_opt_hist.csv"
        , col_types = cols()
    )
...
##########################
write_csv(df_pnl,   "gld_monthly_2014_2018_pnl_master.csv")
write_csv(df_trade, "gld_monthly_2014_2018_trade_master.csv")
