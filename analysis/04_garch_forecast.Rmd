---
title: "Ranking Underlyings with GARCH"
author: "Pritam Dalal"
date: "3/15/2019"
...
# using a for-loop to read all managed PNL data and bind them 
...
```{r, reading_garch, include=FALSE, cache=TRUE}
# using a for-loop to read all garch forecasts and bind them 
# together into a single dataframe.
df_underlying <- 
    tibble(
        symbol = c("DIA", "EEM", "EWZ", "FXI" ,"GDX", "GLD"
...
df_garch_weekly <- tibble()
df_garch_monthly <- tibble()
for (ix in 1:nrow(df_underlying)){
...
```{r, reading_chain_hist, include=FALSE, cache=TRUE}
# using a for-loop to read all chain histories and bind them 
# together into a single dataframe.
...
df_chain_hist_weekly <- tibble()
df_chain_hist_monthly <- tibble()
for (ix in 1:nrow(df_underlying)){
    ...
    paste0(
        "data_output/monthly/"
        , str_to_lower(chr_underlying)
        , "_monthly_2014_2018_chain_hist.csv"
    )
...
```{r, calc_vol_prem_fcst, cache=TRUE, include=FALSE}
# checking for any garch forecasts that didn't get fit
df_garch_monthly %>% filter(is.na(garch_forecast))

# checking for any garch forecasts that didn't get fit
df_garch_weekly %>% filter(is.na(garch_forecast))

# joining together bid swap rates and garch forecasts and then calculating
# volatility premium forecasts
df_garch_weekly <- 
    df_garch_weekly %>% 
        left_join(
            df_chain_hist_weekly 
        ...
        mutate(
            vol_prem_fcst = bid_swap_rate - garch_forecast
        )
```



```{r, calc_garch_rank, include=FALSE, cache=TRUE}
# for each expiration, ranking each underlying by 
# its volatility premium forecast.
df_garch_weekly <- 
    df_garch_weekly %>% 
        group_by(expiration) %>% 
        mutate(vol_prem_rank = rank(-vol_prem_fcst))

df_garch_weekly <- 
    df_garch_weekly %>% 
        arrange(expiration, vol_prem_rank)
```


In this analysis, we calculate GARCH forecasts for each underlying/expiration combination.  We then calculate a forecasted volatility premium for each underlying, and rank the underlyings using this forecast.  We then compare the performance of different groupings: halves, thirds, fourths, sixths, and twelfths.  

In the case of halves, thirds, and fourth, the results are quite stable and as expected: the groups with higher volatility premiums forecasts have higher performance.  For more granular groupings, the data is more noisy, but the general directional point in the right direction - higher ranking leads to better performance.


### Halves

The simplest grouping of underlyings per expiration is to consider the top-half and bottom-half.
```{r, halves_calc, cache=TRUE, include=FALSE}
# halves
...
As we can see from the following bar graph, there is significantly better performance in the top group as compared to the bottom group, particularly when delta-hedging.



```{r, halves_results, include=TRUE, cache=TRUE, echo=FALSE, fig.height=4, fig.width=6, fig.align="center"}
...
The following bar graph show how many times each underlying was in the top group, this seems pretty strongly correlated with the overall performance of the underlying over the five year period.


```{r, top_count, cache=TRUE, echo=FALSE, fig.height=4}
df_counts <- 
    df_garch_weekly %>% 
...
The following two graphs show that for all loss-trigger levels, the top-half outperforms the bottom half.  It also seems that loss-trigger risk-management is more effective for the top half, but it's hard to know if this is just a random artifact of the data.


```{r, halves_graph_loss_trigger, include=TRUE, cache=TRUE, echo=FALSE}
df_sharpe %>% 
...
It is interesting to check if the out-performance of the top group occurs for each year.  This seems to be mostly the case, but it's not 100% true.  In the case of delta-hedging, for 2017 and 2018, the bottom group performed better.  In the case of naked options, the bottom group performed better in 2018. 

The fact that the top group didn't outperform in all years is a bit dissatisfying.  However, there is probably not enough data to make these results too concerning.  Also, there were some underlyings that were hit much harder in 2018, and those may have had higher forecasted vol premium, just by chance.


```{r, halves_sharpe_by_year, echo=FALSE, cache=TRUE}
df_sharpe_by_year %>%
    filter(loss_trigger == 1000) %>% 
    filter(dh_threshold == 0) %>% 
...
### Thirds
```{r, thirds_calc, include=FALSE, cache=TRUE}
# thirds
my_grouping <- function(rank){
...
When breaking the underlyings into three groups, we mostly see the expected ordering of performance.  However, in the case of naked option selling the middle does worse than the bottom group. 



```{r, thirds_results, echo=FALSE, cache=TRUE, fig.height=4, fig.width=6, fig.align="center"}
df_sharpe %>% 
    filter(loss_trigger == 1000) %>% 
    ggplot(aes(x=factor(vol_prem_group), y=sharpe, fill=factor(vol_prem_group))) +
        geom_text(aes(label=round(sharpe, 2), vjust=-0.25)) + 
        ...
        facet_wrap(~dh_threshold) +
        labs(
            title = "Thirds - sharpe ratio by group"
            , subtitle = "faceted by hedging"
            , fill = "group"
            , x = "group"
        )
...
When graphing the Sharpe by loss-trigger lines, the ordering looks correct. For naked option-selling, the top group seems the most responsive to loss-trigger risk management, but I won't put too much stock in these results unless I see this pattern repeated in subsequent analysis.

```{r, thirds_graphs, echo=FALSE, cache=TRUE}
df_sharpe %>% 
    filter(dh_threshold == 0) %>% 
    filter(loss_trigger != 1000) %>% 
    ggplot(aes(x=loss_trigger, y=sharpe, color = factor(vol_prem_group))) +
...
I also consider groups of fourths, sixths, and twelfths (singleton).  The results get quite a bit noisier, but they are in general directionally correct, so overall I am satisfied that the GARCH-based volatility premium forecast is predictive.


#### Fourths
```{r, fourths_calc, cache=TRUE, include=FALSE}
# fourths
my_grouping <- function(rank){
    if (rank %in% c(1, 2, 3)) {
        g = 1L 
...
```{r, fourths_results, echo=FALSE, cache=TRUE}
df_sharpe %>% 
    filter(loss_trigger == 1000) %>% 
    ggplot(aes(x=factor(vol_prem_group), y=sharpe, fill=factor(vol_prem_group))) +
        geom_text(aes(label=round(sharpe, 2), vjust=-0.25)) + 
        ...
        facet_wrap(~dh_threshold) +
        labs(
            title = "Fourths - sharpe ratio by group"
            , subtitle = "faceted by hedging"
            , fill = "group"
            , x = "group"
        )
...
```{r, fourths_graphs, echo=FALSE, cache=TRUE}
df_sharpe %>% 
    filter(dh_threshold == 0) %>% 
    filter(loss_trigger != 1000) %>% 
    ggplot(aes(x=loss_trigger, y=sharpe, color = factor(vol_prem_group))) +
        ...
        geom_smooth(se = FALSE, method = "loess", formula = "y ~ x") +
        labs(
            title = "Fourths - sharpe by loss-trigger (delta-hedge)"
            , color = "group"
        )
...
df_sharpe %>% 
    ...
    ggplot(aes(x=loss_trigger, y=sharpe, color = factor(vol_prem_group))) +
        ...
        geom_smooth(se = FALSE, method = "loess", formula = "y ~ x") +
        labs(
            title = "Fourths - sharpe by loss-trigger (naked options)"
            , color = "group"
        )
```
...
#### Sixths
```{r, sixths_calc, cache=TRUE, include=FALSE}
# sixths
my_grouping <- function(rank){
    if (rank %in% c(1, 2)) {
        g = 1L 
...
```{r, sixths_results, echo=FALSE, cache=TRUE}
df_sharpe %>% 
    filter(loss_trigger == 1000) %>% 
    ggplot(aes(x=factor(vol_prem_group), y=sharpe, fill=factor(vol_prem_group))) +
        geom_text(aes(label=round(sharpe, 2), vjust=-0.25)) + 
        ...
        facet_wrap(~dh_threshold) +
        labs(
            title = "Sixths - sharpe ratio by group"
            , subtitle = "faceted by hedging"
            , fill = "group"
            , x = "group"
        )
...
```{r, sixth_graphs, echo=FALSE, cache=TRUE}
...
#### Twelfths
```{r, twelfths_calc, cache=TRUE, include=FALSE}
# twelfths
my_grouping <- function(rank){
    g = as.integer(rank)
    g
...
```{r, twelfths_results, echo=FALSE, cache=TRUE}
df_sharpe %>% 
    filter(loss_trigger == 1000) %>% 
    ggplot(aes(x=factor(vol_prem_group), y=sharpe, fill=factor(vol_prem_group))) +
        geom_text(aes(label=round(sharpe, 2), vjust=-0.25)) + 
        ...
        facet_wrap(~dh_threshold) +
        labs(
            title = "Twelfths - sharpe ratio by group"
            , subtitle = "faceted by hedging"
            , fill = "group"
            , x = "group"
        )
...
```{r, twelfths_graphs, echo=FALSE, cache=TRUE}
df_sharpe %>% 
    filter(dh_threshold == 0) %>% 
    filter(loss_trigger != 1000) %>% 
    ggplot(aes(x=loss_trigger, y=sharpe, color = factor(vol_prem_group))) +
        geom_smooth(se = FALSE, method = "loess", formula = "y ~ x") +
        labs(
            title = "Twelfths - sharpe by loss-trigger (delta-hedge)"
            , color = "ranking"
        )
...
df_sharpe %>% 
    ...
    ggplot(aes(x=loss_trigger, y=sharpe, color = factor(vol_prem_group))) +
        geom_smooth(se = FALSE, method = "loess", formula = "y ~ x") +
        labs(
            title = "Twelfths - sharpe by loss-trigger (naked)"
            , color = "ranking"
        )
```
...
These results demonstrate that GARCH based volatility estimates do provide information about future estimates about volatility.  I have been trading for a couple of months using these estimates to pick the underlyings, and I will continue to do so.

In another analysis it would be interesting to show that with monthly options, there is less information in the GARCH estimates.

<!-- ```{r, cache=TRUE} -->
<!-- df_top_fcst <-  -->
<!--     df_garch_weekly %>%  -->
<!--         filter(vol_prem_rank <= 6) -->
...
<!--     df_pnl_weekly %>%  -->
<!--         inner_join( -->
<!--             df_top_fcst %>% select(expiration, underlying) -->
<!--             , by = c("expiration", "underlying") -->
<!--         )  -->
...
<!--     df_pnl_weekly %>%  -->
<!--         inner_join( -->
<!--             df_bottom_fcst %>% select(expiration, underlying) -->
<!--             , by = c("expiration", "underlying") -->
<!--         )  -->
